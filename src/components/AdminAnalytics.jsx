...
export default function AdminAnalytics({ filas, configuracion }) {
  ...
  const data = useMemo(() => {
    ...
    const vecinos = filasPorAnio.map((fila, index) => crearVecino(fila, index, configuracion));
  ...
  const vecinosFiltrados = useMemo(() => {
    ...
    const base = data.vecinos
      ...
      .filter((vecino) => {
        const coincideTexto =
          !termino ||
          normalizarTexto(vecino.nombre).includes(termino) ||
          normalizarTexto(vecino.parcela).includes(termino) ||
          normalizarTexto(vecino.sitio).includes(termino);

        const coincideNivelPago =
          filtroNivelPago === 'todos' || filtroNivelPago === vecino.nivelPago;

        return coincideTexto && coincideNivelPago;
      });
    ...
    return sorted;
  }, [busqueda, data.vecinos, filtroNivelPago, orden, filtroMes]);
    ...
    <div className="bg-white rounded-[2rem] shadow-xl p-4 md:p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-1 text-lg font-bold text-slate-900">{analiticaFiltrada.totalVecinos}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-xs text-slate-500">Recaudado</p>
          <p className="mt-1 text-lg font-bold text-slate-900">
            {formatearMonto(analiticaFiltrada.totalPagado)}
          </p>
          <p className="text-xs text-slate-500 mt-1">
            / {formatearMonto(analiticaFiltrada.totalPendiente)} faltante
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-xs text-slate-500">Cuotas extra</p>
          <p className="mt-1 text-lg font-bold text-amber-700">
            {formatearMonto(analiticaFiltrada.totalCuotasExtra)}
          </p>
          <p className="text-xs text-slate-500 mt-1">
            / {formatearMonto(analiticaFiltrada.totalPendienteCuotasExtra)} faltante
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-xs text-slate-500">Cumplimiento meses</p>
          <p className="mt-1 text-lg font-bold text-slate-900">
            {formatearPorcentaje(analiticaFiltrada.porcentajeCumplimientoMeses)}
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-xs text-slate-500">Vecinos al dia</p>
          <p className="mt-1 text-lg font-bold text-emerald-700">{analiticaFiltrada.totalAlDia}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-xs text-slate-500">Vecinos parciales</p>
          <p className="mt-1 text-lg font-bold text-amber-700">{analiticaFiltrada.totalParciales}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-xs text-slate-500">Vecinos pendientes</p>
          <p className="mt-1 text-lg font-bold text-red-600">{analiticaFiltrada.totalPendientes}</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 p-4 space-y-3">
        <div className="flex flex-wrap gap-2">
          ...
          <select
            ...
            className="rounded-xl border border-slate-300 px-3 py-2 text-sm"
          >
            <option value="TODOS">Anio: Todos</option>
          ...
          <input
            ...
            placeholder="Buscar por nombre, parcela o sitio"
          ...
          >
            ...
            <option value="pendiente_asc">Menor pendiente</option>
            <option value="pagado_asc">Menor pagado</option>
          </select>
        </div>

        <div className="text-xs text-slate-500">
          Filtro activo: {filtroMes === 'TODOS' ? 'Todos los meses' : filtroMes} |{' '}
        ...
        <div className="overflow-x-auto border border-slate-200 rounded-2xl">
          <table className="min-w-full text-xs md:text-sm">
            <thead className="bg-slate-100 text-slate-700">
              <tr>
                <th className="px-3 py-2 text-left">Vecino</th>
                <th className="px-3 py-2 text-left">Parcela / Sitio</th>
                <th className="px-3 py-2 text-left">Estado</th>
                <th className="px-3 py-2 text-left">Nivel pago</th>
                <th className="px-3 py-2 text-right">Pagado</th>
                <th className="px-3 py-2 text-right">Pendiente</th>
                {filtroMes !== 'TODOS' ? (
                  <>
                    <th className="px-3 py-2 text-right">{`Pagado ${filtroMes}`}</th>
                    <th className="px-3 py-2 text-right">{`Pendiente ${filtroMes}`}</th>
                  </>
                ) : null}
                <th className="px-3 py-2 text-right">Meses completos</th>
            ...
            <tbody>
              {vecinosPagina.map((vecino) => (
                <tr key={vecino.id} className="border-t border-slate-200">
                  <td className="px-3 py-2 text-slate-900 font-medium">{vecino.nombre}</td>
