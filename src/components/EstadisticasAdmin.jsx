...
export default function EstadisticasAdmin({ filas, configuracion }) {
  ...
  const estadisticas = useMemo(() => {
    ...
    const totalCuotasExtra = vecinos.reduce(
      (acc, vecino) =>
        acc + vecino.cuotasExtra.reduce((subtotal, cuota) => subtotal + cuota.monto, 0),
      0
    );
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-xl p-4 md:p-6 space-y-4">
      <button
        ...
        className="w-full flex items-center justify-between gap-3 rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 text-left"
      >
        <div>
          ...
          <p className="mt-1 text-sm md:text-base font-semibold text-slate-900">
            Totales generales, meses y parcelas con mas aportes
          </p>
        </div>
